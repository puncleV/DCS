...
int processCountGlobal, eventsLog, pipesLog, currentReceivedId, useMutex = 0;
...
int request_cs(const void * self) {
    ...
    int replyCount = 0;
    timestamp_t msgTimestamp;
    msgTimestamp= messageToSend.s_header.s_local_time;
    ...
    for(;;) {
        ...
        if( receivedMessageType  == CS_REQUEST ) {
            if( ( recv_msg.s_header.s_local_time < delta ) ||
                    ( ( recv_msg.s_header.s_local_time == delta ) && ( currentReceivedId < currentLocalId ) ) ) {
        ...
        if( receivedMessageType == CS_REPLY ) {
            replyCount++;
            if ( replyCount == (processCountGlobal - 2) ) break;
        }
...
int release_cs(const void * self) {
    local_id currentLocalId = *(local_id *)self;
    int destinationLocalId;
    while ( (destinationLocalId = getLastCs()) ) {
        Message messageToSend = fillMessageHeader(CS_RELEASE, 0);
        clearQueuePending(currentLocalId);
        send(&id, destinationLocalId, &messageToSend);
    }
    return 0;
}
